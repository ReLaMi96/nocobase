...
export default class N8nWebhookStorage extends StorageType {
        ...
        const uploadToN8n = async () => {
          try {
            ...
            if (req && req.headers) {
              ...
              if (cookieHeader) {
                console.log('Upload: Found cookies, length:', cookieHeader.length);
                // Extract session-related cookies
                const sessionMatch = cookieHeader.match(/nocobase[^=]*=([^;]+)/i);
                if (sessionMatch && !userToken) {
                  userToken = sessionMatch[1];
                  console.log('Upload: Found session token:', userToken.substring(0, 20) + '...');
                }
              }*/
              
              // Check for custom headers that might contain user info
              const customHeaders = [
                'x-user-id',
                'x-user-token',
                'x-session-id',
              ...
              console.log('Upload: Available headers:', Object.keys(req.headers).filter(h => 
            ...
            correlationId = `${file.originalname}-${Date.now()}`;
            ...
            formData.append('originalFilename', file.originalname); // for filename-based lookup
            ...
            console.log('  - userId:', userId ? 'encrypted-' + UserIdEncryption.encrypt(userId).substring(0, 16) + '...' : 'lookup-via-token');
            ...
            cb(null, {
              ...
              url: result.url || '',
              key: result.key || result.filename || file.originalname,
            });
          } catch (error) {
            cb(error);
          }
  ...
  async delete(records: AttachmentModel[]): Promise<[number, AttachmentModel[]]> {
    ...
    for (const record of records) {
      try {
        await axios.post(this.storage.options.deleteWebhookUrl, {
  ...
  async getFileStream(file: AttachmentModel, ctx?: any): Promise<{ stream: Readable; contentType?: string }> {
    try {
      ...
      if (!fileUrl && this.storage.options.downloadWebhookUrl) {
        ...
        if (!userId) {
          try {
            ...
            if (globalProcess._nocobaseCurrentContext && globalProcess._nocobaseCurrentContext.state && globalProcess._nocobaseCurrentContext.state.currentUser) {
      ...
      throw new Error('No download method available - please configure downloadWebhookUrl or ensure files have direct URLs');
    } catch (error) {
      const errorMessage = error instanceof Error ? error.message : String(error);
    ...
    if (this.storage.options.downloadWebhookUrl) {
      ...
      if (!urlUserId) {
        ...
        try {
          ...
          if (globalThis._nocobaseCurrentContext && globalThis._nocobaseCurrentContext.state && globalThis._nocobaseCurrentContext.state.currentUser) {
      ...
      if (urlUserId) {
        ...
        console.log('getFileURL: Added encrypted userId with expiration to URL params:', encryptedUserId.substring(0, 20) + '...');
      } else {
